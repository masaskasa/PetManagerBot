package handler

const (
	msgHello            = "Hi there!👋\n\n" + msgHowToBegin
	msgAskName          = "What is your pet's name?"
	msgAskSpecies       = "What is your pet's species? Choose the correct code\n\n"
	msgInvalidSpecies   = "Invalid species. Please choose the correct code\n\n"
	msgAskBreed         = "What is your pet's breed? Choose the correct code\n\n"
	msgInvalidBreed     = "Invalid breed. Please choose the correct code\n\n"
	msgAskSex           = "What is your pet's gender?\n\n/female\n/male"
	msgInvalidSex       = "Invalid gender. Please choose the correct\n\n/female\n/male"
	msgAskAnimalID      = "What is your pet's AnimalID?\n\n/skip"
	msgAskSpecialSigns  = "Specify the special characteristics of the pet\n\n/skip"
	msgConfirmCreatePet = "Your pet is ready! if everything is correct, confirm the creation of a pet\n\n/confirm\n/do_not_confirm\n\n"
	msgReadyCreatePet   = "Your pet is saved!"
	msgTryAgain         = "Sorry, try again "
	msgUnknownCommand   = "Unknown command 🚫"
	msgBreakCommand     = "Operation cancelled\n\n"

	msgHowToBegin = `How to begin?

- Create an animal card /create_pet`
	msgHelp = `TODO help message`
)
